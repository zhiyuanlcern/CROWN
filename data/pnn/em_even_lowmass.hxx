//Code generated automatically by TMVA for Inference of Model file [em_even_lowmass.h5] at [Fri Nov 24 04:38:11 2023] 

#ifndef TMVA_SOFIE_EM_EVEN_LOWMASS
#define TMVA_SOFIE_EM_EVEN_LOWMASS
...
namespace TMVA_SOFIE_em_even_lowmass{
namespace BLAS{
...
std::vector<float> fTensor_dense2kernel0 = std::vector<float>(600);
float * tensor_dense2kernel0 = fTensor_dense2kernel0.data();
std::vector<float> fTensor_dense1bias0 = std::vector<float>(600);
float * tensor_dense1bias0 = fTensor_dense1bias0.data();
std::vector<float> fTensor_dense1kernel0 = std::vector<float>(360000);
float * tensor_dense1kernel0 = fTensor_dense1kernel0.data();
std::vector<float> fTensor_densebias0 = std::vector<float>(600);
float * tensor_densebias0 = fTensor_densebias0.data();
std::vector<float> fTensor_densekernel0 = std::vector<float>(15600);
...
std::vector<float> fTensor_dropout1Identity0 = std::vector<float>(600);
float * tensor_dropout1Identity0 = fTensor_dropout1Identity0.data();
std::vector<float> fTensor_dense1BiasAdd0 = std::vector<float>(600);
float * tensor_dense1BiasAdd0 = fTensor_dense1BiasAdd0.data();
std::vector<float> fTensor_dense1bias0bcast = std::vector<float>(600);
float * tensor_dense1bias0bcast = fTensor_dense1bias0bcast.data();
std::vector<float> fTensor_leakyrelu1LeakyRelu0 = std::vector<float>(600);
float * tensor_leakyrelu1LeakyRelu0 = fTensor_leakyrelu1LeakyRelu0.data();
std::vector<float> fTensor_dropoutIdentity0 = std::vector<float>(600);
float * tensor_dropoutIdentity0 = fTensor_dropoutIdentity0.data();
std::vector<float> fTensor_leakyreluLeakyRelu0 = std::vector<float>(600);
float * tensor_leakyreluLeakyRelu0 = fTensor_leakyreluLeakyRelu0.data();
std::vector<float> fTensor_denseBiasAdd0 = std::vector<float>(600);
...
std::vector<float> fTensor_densebias0bcast = std::vector<float>(600);
float * tensor_densebias0bcast = fTensor_densebias0bcast.data();


Session(std::string filename ="") {
   if (filename.empty()) filename = "data/pnn/em_even_lowmass.dat";
   std::ifstream f;
   f.open(filename);
   if (!f.is_open()){
   ...
   if (length != 600) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 600 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
   ...
   if (length != 360000) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 360000 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
   ...
   if (length != 15600) {
      std::string err_msg = "TMVA-SOFIE failed to read the correct tensor size; expected size is 15600 , read " + std::to_string(length) ;
      throw std::runtime_error(err_msg);
    }
    for (int i =0; i < length; ++i) 
   ...
   f.close();
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_densebias0,{ 600 }, { 1 , 600 });
      std::copy(data, data + 600, tensor_densebias0bcast);
      delete [] data;
   }
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense1bias0,{ 600 }, { 1 , 600 });
      std::copy(data, data + 600, tensor_dense1bias0bcast);
      delete [] data;
   }
   {
...
//--------- Gemm
   ...
   int op_0_lda = 26;
   int op_0_ldb = 600;
   std::copy(tensor_densebias0bcast, tensor_densebias0bcast + 600, tensor_denseBiasAdd0);
...
//--------- Gemm
   ...
   int op_3_lda = 600;
   int op_3_ldb = 600;
   std::copy(tensor_dense1bias0bcast, tensor_dense1bias0bcast + 600, tensor_dense1BiasAdd0);
...
} //TMVA_SOFIE_em_even_lowmass

#endif  // TMVA_SOFIE_EM_EVEN_LOWMASS
